...
# one run for all 4
	for(i in (1:nrow(data)))
	{
	 ...
	 ### Terpenes y <- 4
	  ...
	  ## Broyat = ASAP
	  if(grepl("Broyat", data$Nature[i])==TRUE){
		data$x[i] = 2
	  }
...
# use table(data$x) for checking if the right ionizations are available
	table(data$x)
# if smtg goes wrong i.e. extra data are present with x = 4, but not wished for plotting
...
#  publication 1 / [publication 2 supplementary info] please use publi1 script
...
#	data3 <- subset(data2, ((Polarité=="POS")&(Nature=="Multi 2"))==TRUE)
#	4d / S1d : Nature=="Multi 2" & Polarité =="NEG"
#	data3 <- subset(data2, ((Polarité=="NEG")&(Nature=="Multi 2"))==TRUE)


#  publication 2  mode 3
#	3a	: Nature!="Multi 1" & Polarité =="POS" or ((Nature=="Thalle")|(Nature=="Broyat")&(Polarité=="POS"))  
	data3 <- subset(data2, (((Nature=="Thalle")|(Nature=="Broyat"))==TRUE&(Polarité=="POS")==TRUE)|((x==3)==TRUE))
#	3b  : Nature!="Multi 1" & Polarité =="NEG" or ((Nature=="Thalle")|(Nature=="Broyat")&(Polarité=="NEG")) 
	data3 <- subset(data2, (((Nature=="Thalle")|(Nature=="Broyat"))==TRUE&(Polarité=="NEG")==TRUE)|((x==3)==TRUE))


#  publication 2  mode 4
#	3a	: Nature!="Multi 1" & Polarité =="POS" or ((Nature=="Thalle")|(Nature=="Broyat")&(Polarité=="POS"))  
	data3 <- subset(data2, ((Polarité=="POS")==TRUE)|((x==4)==TRUE))
#	3b  : Nature!="Multi 1" & Polarité =="NEG" or ((Nature=="Thalle")|(Nature=="Broyat")&(Polarité=="NEG")) 
	data3 <- subset(data2, ((Polarité=="NEG")==TRUE)|((x==4)==TRUE))
...
######################################################################################################################
	...
	hist3D_fancy<- function(x, y, dataset, break.func = c("Sturges", "scott", "FD"), breaks = NULL,
	 ...
	 ymid <- 0.5*(y.bin[-1] + y.bin[-length(y.bin)])
	  ...
	  labels <- c( 'DART-MS','ASAP-MS', 'Database')
	  label.pos <- trans3d(x.axis-0.5, (min.y - 1.9), min.z, H3D)
	  #test
	  #text(label.pos$x, label.pos$y, labels=labels[min(x):max(x)], adj=c(0, NA), cex=0.75, las=1)
	  text(label.pos$x, label.pos$y, labels=labels[min(x):max(x)], adj=c(0, NA), cex=0.9, las=1)
	  ...
	  text(z.title.pos$x, z.title.pos$y, labels=z.title, adj=c(1, NA), cex=1.5, srt=102)
