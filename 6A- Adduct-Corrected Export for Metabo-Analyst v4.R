...
setwd(input_dir)
input_files <- list.files(input_dir, pattern = "preVKs[.]csv")
...
for(i in 1:length(input_files)){
  ...
  Xtract[,"mz"]=cbind(csv_active$"EM")
  Xtract[,"into"]=cbind(csv_active$"into")
  
  Xtractfile <- paste(substr(input_files[[i]],1,nchar(input_files[[i]])-17), "_forMA.csv",sep ="")
