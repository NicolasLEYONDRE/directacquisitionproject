...
# Set Directory
input_dir <- 'C:/Users/Simon Ollivier/Desktop/working directory/3_preVKs'
setwd(input_dir)
...
pb <- tkProgressBar(title = "progress bar", min = 0, max = nrow(data))

for(i in (1:nrow(data))){
  HC = as.numeric(data[i,"H.C"])
  OC = as.numeric(data[i,"O.C"])
  
  ## Correlates index w/ chemical family (rectangles approx.)
  
  ### Unsaturated hydrocarbons
  if((between(HC,0.85,1.5)==TRUE)&(between(OC,0,0.1)==TRUE)) data[i,"chem"] = "Unsaturated hydrocarbons"
  
  ### Condensed aromatic compounds
  if((between(OC,0,0.65)==TRUE)&(between(HC,0,0.65)==TRUE)) data[i,"chem"] = "Condensed aromatic compounds"
  if((between(OC,0,0.5)==TRUE)&(between(HC,0,0.7)==TRUE)) data[i,"chem"] = "Condensed aromatic compounds"
  if((between(OC,0,0.45)==TRUE)&(between(HC,0,0.75)==TRUE)) data[i,"chem"] = "Condensed aromatic compounds"
  if((between(OC,0,0.4)==TRUE)&(between(HC,0,0.85)==TRUE)) data[i,"chem"] = "Condensed aromatic compounds"
  
  ### (Poly)phenolic compounds
  if((between(HC,0.65,0.75)==TRUE)&(between(OC,0.45,0.8)==TRUE)) data[i,"chem"] = "Phenolic compounds"
  if((between(HC,0.75,0.85)==TRUE)&(between(OC,0.4,0.85)==TRUE)) data[i,"chem"] = "Phenolic compounds"
  if((between(HC,0.85,1.0)==TRUE)&(between(OC,0.35,0.9)==TRUE)) data[i,"chem"] = "Phenolic compounds"
  if((between(HC,1.0,1.15)==TRUE)&(between(OC,0.385,0.9)==TRUE)) data[i,"chem"] = "Phenolic compounds"
  if((between(HC,1.15,1.30)==TRUE)&(between(OC,0.385,0.85)==TRUE)) data[i,"chem"] = "Phenolic compounds"
  
  ### Terpenoids
  if((between(HC,0.85,1.0)==TRUE)&(between(OC,0.225,0.35)==TRUE)) data[i,"chem"] = "Benzenoids or atypic terpenoids"
  if((between(HC,1.0,1.15)==TRUE)&(between(OC,0.20,0.385)==TRUE)) data[i,"chem"] = "Benzenoids or atypic terpenoids"
  if((between(HC,1.15,1.5)==TRUE)&(between(OC,0.20,0.4)==TRUE)) data[i,"chem"] = "Benzenoids or atypic terpenoids"
  if((between(HC,1.5,1.6)==TRUE)&(between(OC,0.225,0.395)==TRUE)) data[i,"chem"] = "Benzenoids or atypic terpenoids"
  if((between(HC,1.6,1.75)==TRUE)&(between(OC,0.25,0.385)==TRUE)) data[i,"chem"] = "Benzenoids or atypic terpenoids"
  
  ### Nucleic acids
  if((between(HC,1.30,1.50)==TRUE)&(between(OC,0.40,1.05)==TRUE)) data[i,"chem"] = "Nucleic acids"
  if((between(HC,1.50,1.60)==TRUE)&(between(OC,0.60,0.90)==TRUE)) data[i,"chem"] = "Nucleic acids"
  
  ### Steroids
  if((between(HC,1.50,2.00)==TRUE)&(between(OC,0,0.10)==TRUE)) data[i,"chem"] = "Terpenoids"
  
  ## Fatty acids
  if((between(HC,1.75,2.00)==TRUE)&(between(OC,0.1,0.4)==TRUE)) data[i,"chem"] = "Fatty acyls"
  if((between(HC,2.00,2.30)==TRUE)&(between(OC,0,0.4)==TRUE)) data[i,"chem"] = "Fatty acyls"
  
  ## Small acids
  if((between(HC,1.5,1.6)==TRUE)&(between(OC,0.1,0.225)==TRUE)) data[i,"chem"] = "Prenol derivatives"
  if((between(HC,1.6,1.85)==TRUE)&(between(OC,0.1,0.25)==TRUE)) data[i,"chem"] = "Prenol derivatives"
  
  ## Amino acids
  if((between(HC,1.5,1.6)==TRUE)&(between(OC,0.395,0.60)==TRUE)) data[i,"chem"] = "Amino acids"
  if((between(HC,1.6,1.75)==TRUE)&(between(OC,0.385,0.70)==TRUE)) data[i,"chem"] = "Amino acids"
  if((between(HC,1.75,1.9)==TRUE)&(between(OC,0.32,0.82)==TRUE)) data[i,"chem"] = "Amino acids"
  if((between(HC,1.9,2.6)==TRUE)&(between(OC,0.35,0.82)==TRUE)) data[i,"chem"] = "Amino acids"
  
  ## Carbohydrates
  if((between(HC,1.50,1.60)==TRUE)&(between(OC,0.90,1.20)==TRUE)) data[i,"chem"] = "Carbohydrates"
  if((between(HC,1.60,2.50)==TRUE)&(between(OC,0.82,1.20)==TRUE)) data[i,"chem"] = "Carbohydrates"
  
  setTkProgressBar(pb, i, label=paste( round(i/nrow(data)*100, 0), "% done"))
}
...
for (j in 1:nrow(data)){
  ...
  if(grepl("Benzenoids or atypic terpenoids", data$chem[j])==TRUE){
    data$y[j] = 3
  } 
  ...
  if(grepl("Fatty acyls", data$chem[j])==TRUE){
    data$y[j] = 6
  }
  if(grepl("Prenol derivatives", data$chem[j])==TRUE){
    data$y[j] = 7
  }
  ...
  setTkProgressBar(pb, j, label=paste( round(j/nrow(data)*100, 0), "% done"))
  }
close(pb)
...
hist3D_fancy<- function(x, y, dataset, break.func = c("Sturges", "scott", "FD"), breaks = NULL,
  ...
  ##Y
  labels <- c('Condensed aromatic compounds', 'Phenolic compounds', 'Benzenoids', 'Terpenoids',
              'Unsaturated hydrocarbons', 'Fatty acyls', 'Prenyl derivatives', 'Nucleic acids', 'Amino-acids', 'Carbohydrates')
...
##########################################################################################################################
# ep_pos_t <- filter(dataset1, Polarité=="NEG" & (Nature=="Thalle"|Nature=="Broyat"))
